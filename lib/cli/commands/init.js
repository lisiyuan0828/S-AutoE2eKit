 ...
 *     7) 引导安装 auto-e2e Claude skill
 *
 * 流程控制：
 *   - 默认交互式（一次 confirm）；--yes / --auto 跳过
 *   - --manual 不真执行，只打印手动命令
 *   - --dry-run 走全流程但不真改文件、不真装包
 *   - --only=<id> 只跑某一步（deps / browsers / config / docs / scripts / skill）
 *   - --skip-browsers / --skip-skill 跳过对应步骤
 *
 * 失败策略：
...
const ensureSkill = require('../steps/ensure-skill');
...
async function init(opts = {}) {
  ...
  const checklist = [
    ...
    { id: 'docs',     label: '创建 docs/e2e/ 业务文档骨架（5 个文件）', skip: false,                 fn: ensureDocs },
    { id: 'scripts',  label: '注入 package.json scripts (e2e/e2e:ui)',  skip: false,                 fn: ensureScripts },
    { id: 'skill',    label: '引导安装 auto-e2e Claude skill（可选）',   skip: !!opts.skipSkill,       fn: ensureSkill },
  ];
  ...
  logger.plain(`  ${c.cyan('npm run e2e:headed')}       ${c.dim('# 带浏览器窗口跑（debug）')}`);
  logger.plain(`  ${c.cyan('npm run e2e:report')}       ${c.dim('# 打开 HTML 报告')}`);
  logger.blank();
  logger.plain(c.bold('体检环境:'));
  logger.plain(`  ${c.cyan('npx s-auto-e2e-kit doctor')}`);
  logger.blank();
  logger.hint('在 Claude / CodeBuddy 里直接说 "测一下登录流程"，让 skill 自动接管。');
  logger.blank();
}
